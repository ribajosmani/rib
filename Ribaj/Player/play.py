...
async def play(c: Client, m: Message):
    ...
    if replied:
        ...
        if len(m.command) < 2:
         await m.reply_photo(
                     photo=f"{IMG_5}",
                    caption="**Usage: /play Give a Title Song To Play Music or /vplay for Video Play Wixii Intaa Dheer la Xariir @ribajosmani**",
        ...
        f"**𝐑𝐚𝐝𝐢𝐧 𝐁𝐲 𝐑𝐢𝐛𝐚𝐣 𝐎𝐬𝐦𝐚𝐧𝐢 Downloading**\n\n100% ▓▓▓▓▓▓▓▓▓▓▓▓ 00%"
    )
            query = m.text.split(None, 1)[1]
            search = ytsearch(query)
            if search == 0:
                await suhu.edit("🤦‍♂️ **Natiijo Aan La Helin Isku day markale.**")
            else:
                songname = search[0]
                title = search[0]
                url = search[1]
                ...
                if abhi == 0:
                    ...
                    if chat_id in QUEUE:
                        ...
                        try:
                            await suhu.edit(
                            f"**𝐃𝐚𝐚𝐫𝐦𝐚𝐲 𝐁𝐲 𝐎𝐬𝐦𝐚𝐧𝐢 𝐁𝐨𝐭 Downloader**\n\n**Title**: {title[:22]}\n\n0% ████████████100%\n\n**Time Taken**: 00:00 Seconds\n\n**Converting Audio[FFmpeg Process]**"
                        )
                            await call_py.join_group_call(
                                chat_id,
                                AudioPiped(
                            ...
                            await suhu.delete()
                            await m.reply_text(f"💬 Dalabkaaga Lama Helin Isku day markale: `{ep}`")
